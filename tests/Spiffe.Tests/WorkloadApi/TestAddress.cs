...
public partial class TestAddress
{
    ...
    private static void AssertParse((string Addr, string Expected, string Err)[] testCases,
        Func<string, string> parseFunc)
    {
        foreach ((string addr, string expected, string err) in testCases)
        {
            ...
            if (assertOk)
            {
                ...
                Assert.True(expected == parseFunc(addr),
                    $"Result mismatch: input='{addr}' expected='{expected}' actual='{actual}'");
            }
            else
            {
                ...
                Assert.True(e.Message.StartsWith(err),
                    $"Error mismatch: input='{addr}' expected='{err}' actual='{e.Message}'");
            }
        }
    }
}
