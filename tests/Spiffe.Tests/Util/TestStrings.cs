...
public class TestStrings
{
    ...
    public void TestToString()
    {
        ...
        X509BundleSet xbs = new(new()
        {
            { td, ca.X509Bundle() },
        });
        JwtSvid j1 = ca.CreateJwtSvid(w1, [w2.Id], "hello");
        JwtBundleSet jbs = new(new()
        {
            { td, ca.JwtBundle() },
        });
        X509Context ctx = new([s1, s2], xbs);

        string s = Strings.ToString(xbs, false);
        ...
        s = Strings.ToString(new X509SVID()
        {
            SpiffeId = w1.Id,
        });
        s.Should().Contain(w1.Id);
