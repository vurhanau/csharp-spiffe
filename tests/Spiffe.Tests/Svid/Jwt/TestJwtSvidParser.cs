...
public class TestJwtSvidParser
{
    ...
    private static readonly IJwtBundleSource s_source =
        new JwtBundleSet(new Dictionary<TrustDomain, JwtBundle> { { s_td, s_ca.JwtBundle() } });
    ...
    public async Task TestValidate()
    {
        ...
        await f.Should().ThrowAsync<JwtSvidException>()
            .WithMessage($"Expected audience is {s_workload1.Id} (audience={s_workload2.Id})");
    }
    ...
    public async Task TestParseFailsIfAudienceInvalid()
    {
        ...
        await fn.Should().ThrowAsync<JwtSvidException>()
            .WithMessage($"Expected audience is {workload3.Id} (audience={s_workload2.Id})");
    }
    ...
    public async Task TestParseFailsIfAlgUnknown()
    {
        ...
        JwtHeader header = new() { ["alg"] = "unknown" };
    ...
    public async Task TestParseFailsIfValidToInvalid()
    {
        ...
        IJwtBundleSource source = new JwtBundleSet(new Dictionary<TrustDomain, JwtBundle> { { s_td, ca.JwtBundle() } });
        DateTime now = DateTime.UtcNow;
        List<Claim> claims =
        [
            new(Claims.Iss, "FAKECA"),
    ...
    public async Task TestParseFailsIfIssuedAtInvalid()
    {
        ...
        Func<Task> fn = async () => await JwtSvidParser.ParseAsync(badJwt, s_source, [s_workload2.Id]);
        await fn.Should().ThrowAsync<JwtSvidException>()
            .WithMessage("Validation failed, token issued in the future (iat)");
    }
    ...
    public async Task TestParseWithInvalidSignature()
    {
        ...
        JwtSvidException e =
            await Assert.ThrowsAsync<JwtSvidException>(() =>
                JwtSvidParser.ParseAsync(badJwt, s_source, [s_workload1.Id]));
    ...
    public async Task TestParseFailsIfBundleNotFound()
    {
        ...
        IJwtBundleSource source = new JwtBundleSet(new Dictionary<TrustDomain, JwtBundle> { { td2, ca2.JwtBundle() } });
    ...
    public async Task TestParseFailsIfKeyNotFound()
    {
        ...
        IJwtBundleSource source = new JwtBundleSet(new Dictionary<TrustDomain, JwtBundle> { { td1, ca2.JwtBundle() } });
        string jwt = ca1.CreateJwtSvid(workload1, [workload2.Id]).Token;
