...
public static class JwtSvidParser
{
    ...
    public static async Task<JwtSvid> Parse(string token, IJwtBundleSource bundleSource, IEnumerable<string> audience)
    {
        return await Parse(token, audience, async (jwt, td) =>
        {
            ...
            TokenValidationResult result = await ValidateTokenAsync(jwt, key, audience);
            if (!result.IsValid)
            {
    ...
    /// Validates JWT token.
    /// </summary>
    public static async Task<TokenValidationResult> ValidateTokenAsync(JsonWebToken jwt,
